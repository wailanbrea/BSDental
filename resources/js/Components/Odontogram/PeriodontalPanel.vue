...
<template>
    <section class="min-w-0 overflow-hidden border border-[#D8E0DE] bg-white shadow-[0_4px_12px_rgba(15,23,42,0.04)]">
        ...
        <form class="min-w-0" @submit.prevent="submit">
            <div class="max-w-full overflow-x-auto" role="region" aria-label="Mediciones periodontales por sitio" tabindex="0">
            ...
            <div class="flex flex-col items-stretch gap-3 border-t border-[#D8E0DE] p-3 sm:flex-row sm:items-center sm:justify-between"><p class="text-[11px] text-[#64748B]">Valores 4 mm o mayores requieren evaluación periodontal.</p><button type="submit" :disabled="form.processing" class="inline-flex h-9 items-center justify-center gap-2 bg-[#005C55] px-4 text-xs font-bold text-white disabled:opacity-60"><Save class="h-3.5 w-3.5" />Guardar sondaje</button></div>
        </form>
    </section>
</template>
