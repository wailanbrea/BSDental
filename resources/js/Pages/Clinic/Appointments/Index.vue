...
<template>
    ...
    <ClinicLayout>
        ...
        <div v-if="isBlockModal" class="fixed inset-0 z-50 grid place-items-center bg-[#0F172A]/35 p-4" @click.self="isBlockModal = false"><form class="w-full max-w-xl rounded-lg bg-white shadow-2xl" @submit.prevent="submitBlock"><header class="flex items-center justify-between border-b border-[#D8E0DE] px-5 py-4"><div><h2 class="font-bold text-[#131B2E]">Bloquear horario</h2><p class="text-xs text-[#64748B]">Reserva un espacio para reunión, mantenimiento o indisponibilidad.</p></div><button type="button" @click="isBlockModal = false"><X class="h-5 w-5" /></button></header><div class="grid gap-4 p-5 md:grid-cols-2">
        ...
        </div><footer class="flex justify-end gap-2 border-t border-[#D8E0DE] p-4"><button type="button" class="h-10 border border-[#BDC9C6] px-4" @click="isBlockModal = false">Cancelar</button><button :disabled="blockForm.processing" class="h-10 bg-[#005C55] px-4 font-semibold text-white disabled:opacity-60">Crear bloqueo</button></footer></form></div>

        <div v-if="selectedAppointment" class="fixed inset-0 z-50 flex justify-end bg-[#0F172A]/35 backdrop-blur-sm" @click.self="selectedAppointment = null"><aside class="h-full w-full max-w-xl overflow-y-auto border-l border-[#BDC9C6] bg-white shadow-2xl">
