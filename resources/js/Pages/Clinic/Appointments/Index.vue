...
interface AppointmentItem {
    ...
    start_time: string; end_time: string; duration_minutes: number; status: string; priority: 'low' | 'normal' | 'high' | 'urgent'; reason: string | null
...
interface AppointmentHistoryItem { appointment_id: string; user_name: string; old_status: string | null; new_status: string; reason: string | null; created_at: string }
...
const props = defineProps<{
    ...
    appointments: AppointmentItem[]; blocks: ScheduleBlockItem[]; appointmentHistory: AppointmentHistoryItem[]
...
const newAppointmentForm = useForm({
    ...
    room_id: '', appointment_type_id: props.appointmentTypes[0]?.id || '', start_time: `${props.filters.date}T09:00`, duration_minutes: 30, priority: 'normal', reason: '',
})
...
function statusLabel(status: string) {
    return ({ scheduled: 'Programada', pending_confirmation: 'Pendiente de confirmación', confirmed: 'Confirmada', checked_in: 'Registrado', waiting: 'En espera', in_progress: 'En consulta', completed: 'Completada', cancelled: 'Cancelada', no_show: 'No asistió', rescheduled: 'Reprogramada' } as Record<string, string>)[status] || status
}
function priorityLabel(priority: string) { return ({ low: 'Baja', normal: 'Normal', high: 'Alta', urgent: 'Urgente' } as Record<string, string>)[priority] || priority }
function historyFor(appointmentId: string) { return props.appointmentHistory.filter((item) => item.appointment_id === appointmentId) }
</script>
                    ...
                    <div class="grid" :style="{ gridTemplateColumns: `88px repeat(${visibleDays.length}, minmax(120px, 1fr))` }">
                        ...
                        <div v-for="day in visibleDays" :key="isoDate(day)" class="relative border-r border-[#D8E0DE] last:border-0" :style="{ height: `${hours.length * hourHeight}px`, backgroundImage: `repeating-linear-gradient(to bottom, transparent 0, transparent ${hourHeight - 1}px, #E1E7E5 ${hourHeight}px)` }" @dblclick="openCreate(day)">
                            ...
                            <article v-for="item in appointmentsForDay(day)" :key="item.id" class="absolute left-1 right-1 z-20 cursor-pointer overflow-hidden border-l-4 px-2 py-1 text-xs shadow-sm hover:ring-2 hover:ring-[#005C55]/25" :class="eventClass(item.status)" :style="eventStyle(item)" @click.stop="openAppointment(item)">
                                ...
                                <div class="mt-1 flex gap-1"><button v-if="item.status === 'scheduled'" title="Pendiente de confirmación" @click.stop="updateStatus(item, 'pending_confirmation')"><Clock3 class="h-3.5 w-3.5" /></button><button v-if="['scheduled','pending_confirmation','confirmed'].includes(item.status)" title="Check-in" @click.stop="updateStatus(item, 'checked_in')"><UserCheck class="h-3.5 w-3.5" /></button><button v-if="item.status === 'checked_in'" title="En espera" @click.stop="updateStatus(item, 'waiting')"><Clock3 class="h-3.5 w-3.5" /></button><button v-if="['checked_in','waiting'].includes(item.status)" title="Iniciar" @click.stop="updateStatus(item, 'in_progress')"><Play class="h-3.5 w-3.5" /></button><button v-if="item.status === 'in_progress'" title="Finalizar" @click.stop="updateStatus(item, 'completed')"><Check class="h-3.5 w-3.5" /></button><button v-if="!['completed','cancelled','no_show','rescheduled'].includes(item.status)" title="Cancelar" @click.stop="openCancellation(item)"><X class="h-3.5 w-3.5" /></button></div>
                            </article>
                        </div>
                    </div>
                </div>
...
<header class="flex items-center justify-between border-b border-[#D8E0DE] px-5 py-4"><div><h2 class="font-bold text-[#131B2E]">Nueva cita</h2><p class="text-xs text-[#64748B]">Reserva rápida de agenda clínica</p></div><button type="button" @click="isCreatingModal = false"><X class="h-5 w-5" /></button></header><div class="grid gap-4 p-5 md:grid-cols-2">
            ...
            <label><span class="mb-1 block text-xs font-semibold text-[#455653]">Prioridad</span><select v-model="newAppointmentForm.priority" class="h-10 w-full border border-[#BDC9C6] bg-white px-3 text-sm"><option value="low">Baja</option><option value="normal">Normal</option><option value="high">Alta</option><option value="urgent">Urgente</option></select></label>
...
<aside class="h-full w-full max-w-xl overflow-y-auto border-l border-[#BDC9C6] bg-white shadow-2xl">
            ...
            <div class="space-y-5 p-6">
                ...
                <dl class="grid gap-4 rounded-lg border border-[#D8E0DE] p-4 sm:grid-cols-2"><div><dt class="text-xs text-[#64748B]">Fecha y hora</dt><dd class="mt-1 flex items-center gap-2 text-sm font-semibold text-[#263633]"><CalendarClock class="h-4 w-4 text-[#005C55]" /> {{ formatDateTime(selectedAppointment.start_time) }}</dd></div><div><dt class="text-xs text-[#64748B]">Duración</dt><dd class="mt-1 text-sm font-semibold text-[#263633]">{{ selectedAppointment.duration_minutes }} minutos</dd></div><div><dt class="text-xs text-[#64748B]">Profesional</dt><dd class="mt-1 text-sm font-semibold text-[#263633]">{{ selectedAppointment.professional.full_name }}</dd></div><div><dt class="text-xs text-[#64748B]">Prioridad</dt><dd class="mt-1 text-sm font-semibold text-[#263633]">{{ priorityLabel(selectedAppointment.priority) }}</dd></div><div><dt class="text-xs text-[#64748B]">Sillón</dt><dd class="mt-1 flex items-center gap-2 text-sm font-semibold text-[#263633]"><MapPin class="h-4 w-4 text-[#005C55]" /> {{ selectedAppointment.room?.name || 'Sin asignar' }}</dd></div><div class="sm:col-span-2"><dt class="text-xs text-[#64748B]">Motivo / procedimiento</dt><dd class="mt-1 text-sm font-semibold text-[#263633]">{{ selectedAppointment.appointment_type?.name || selectedAppointment.reason || 'Consulta general' }}</dd></div></dl>
                <section class="rounded-lg border border-[#D8E0DE] p-4"><h3 class="text-sm font-semibold text-[#131B2E]">Historial de estado</h3><p v-if="historyFor(selectedAppointment.id).length === 0" class="mt-2 text-xs text-[#64748B]">Aún no hay cambios de estado registrados.</p><ol v-else class="mt-3 space-y-2"><li v-for="entry in historyFor(selectedAppointment.id)" :key="`${entry.created_at}-${entry.new_status}`" class="border-l-2 border-[#005C55] pl-3 text-xs text-[#455653]"><span class="font-semibold text-[#131B2E]">{{ entry.old_status ? `${statusLabel(entry.old_status)} → ` : '' }}{{ statusLabel(entry.new_status) }}</span><span> · {{ entry.user_name }} · {{ formatDateTime(entry.created_at) }}</span><p v-if="entry.reason" class="mt-0.5">{{ entry.reason }}</p></li></ol></section>
            ...
            <footer v-if="!isRescheduling && !isCancelling" class="sticky bottom-0 grid gap-2 border-t border-[#D8E0DE] bg-white px-6 py-4 sm:grid-cols-2"><button v-if="selectedAppointment.status === 'scheduled'" class="inline-flex h-10 items-center justify-center gap-2 bg-[#2458C6] text-sm font-semibold text-white" @click="updateStatus(selectedAppointment, 'pending_confirmation')"><Clock3 class="h-4 w-4" /> Solicitar confirmación</button><button v-if="selectedAppointment.status === 'pending_confirmation'" class="inline-flex h-10 items-center justify-center gap-2 bg-[#2458C6] text-sm font-semibold text-white" @click="updateStatus(selectedAppointment, 'confirmed')"><Check class="h-4 w-4" /> Confirmar cita</button><button v-if="['scheduled','pending_confirmation','confirmed'].includes(selectedAppointment.status)" class="inline-flex h-10 items-center justify-center gap-2 bg-[#2458C6] text-sm font-semibold text-white" @click="updateStatus(selectedAppointment, 'checked_in')"><UserCheck class="h-4 w-4" /> Registrar llegada</button><button v-if="selectedAppointment.status === 'checked_in'" class="inline-flex h-10 items-center justify-center gap-2 bg-[#007D73] text-sm font-semibold text-white" @click="updateStatus(selectedAppointment, 'waiting')"><Clock3 class="h-4 w-4" /> Enviar a espera</button><button v-if="['checked_in','waiting'].includes(selectedAppointment.status)" class="inline-flex h-10 items-center justify-center gap-2 bg-[#007D73] text-sm font-semibold text-white" @click="updateStatus(selectedAppointment, 'in_progress')"><Play class="h-4 w-4" /> Iniciar consulta</button><button v-if="selectedAppointment.status === 'in_progress'" class="inline-flex h-10 items-center justify-center gap-2 bg-[#005C55] text-sm font-semibold text-white" @click="updateStatus(selectedAppointment, 'completed')"><Check class="h-4 w-4" /> Completar</button><button v-if="['scheduled','pending_confirmation','confirmed'].includes(selectedAppointment.status)" class="inline-flex h-10 items-center justify-center gap-2 border border-[#B54708] text-sm font-semibold text-[#B54708]" @click="updateStatus(selectedAppointment, 'no_show')"><X class="h-4 w-4" /> No asistió</button><button v-if="!['completed','cancelled','no_show','rescheduled'].includes(selectedAppointment.status)" class="inline-flex h-10 items-center justify-center gap-2 border border-[#BDC9C6] text-sm font-semibold text-[#263633]" @click="openReschedule(selectedAppointment)"><RotateCcw class="h-4 w-4" /> Reprogramar</button><button v-if="!['completed','cancelled','no_show','rescheduled'].includes(selectedAppointment.status)" class="inline-flex h-10 items-center justify-center gap-2 border border-[#FDA29B] text-sm font-semibold text-[#B42318]" @click="openCancellation(selectedAppointment)"><X class="h-4 w-4" /> Cancelar cita</button></footer>
        </aside>
</div>
    </ClinicLayout>
</template>
