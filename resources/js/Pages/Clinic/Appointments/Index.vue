...
import { Check, ChevronLeft, ChevronRight, Clock3, Filter, LockKeyhole, Play, Plus, UserCheck, X } from 'lucide-vue-next'
...
const props = defineProps<{
    ...
    appointments: AppointmentItem[]; blocks: Array<{ id: string; title: string; start_time: string; end_time: string }>
    filters: { branch_id: string; professional_id: string | null; date: string; view: 'day' | 'week' | 'month' }
}>()
...
function applyFilter() {
    router.get('/appointments', { branch_id: selectedBranch.value, professional_id: selectedProfessional.value || undefined, date: selectedDate.value, view: view.value }, { preserveState: true, replace: true })
}
...
function updateStatus(item: AppointmentItem, status: string) { useForm({ status }).put(`/appointments/${item.id}/status`, { preserveScroll: true }) }
    ...
    <ClinicLayout>
        <div class="space-y-5">
            ...
            <section class="rounded-lg border border-[#BDC9C6] bg-white p-4 shadow-sm">
                ...
                <div v-if="showFilters" class="mt-4 grid gap-3 border-t border-[#D8E0DE] pt-4 md:grid-cols-2">
                    <select v-model="selectedBranch" class="h-10 border border-[#BDC9C6] bg-white px-3 text-sm" @change="applyFilter"><option v-for="branch in branches" :key="branch.id" :value="branch.id">{{ branch.name }}</option></select>
                ...
                <div class="overflow-x-auto"><div class="min-w-[900px]">
                    ...
                    <div class="grid" :style="{ gridTemplateColumns: `88px repeat(${visibleDays.length}, minmax(120px, 1fr))` }">
                        ...
                        <div v-for="day in visibleDays" :key="isoDate(day)" class="relative border-r border-[#D8E0DE] last:border-0" :style="{ height: `${hours.length * hourHeight}px`, backgroundImage: `repeating-linear-gradient(to bottom, transparent 0, transparent ${hourHeight - 1}px, #E1E7E5 ${hourHeight}px)` }" @dblclick="openCreate(day)">
                            <article v-for="item in appointmentsForDay(day)" :key="item.id" class="absolute left-1 right-1 overflow-hidden border-l-4 px-2 py-1 text-xs shadow-sm" :class="eventClass(item.status)" :style="eventStyle(item)">
        ...
        </div><footer class="flex justify-end gap-2 border-t border-[#D8E0DE] px-5 py-4"><button type="button" class="h-10 border border-[#BDC9C6] px-4 text-sm" @click="isCreatingModal = false">Cancelar</button><button :disabled="newAppointmentForm.processing" class="h-10 bg-[#005C55] px-5 text-sm font-semibold text-white disabled:opacity-60">Guardar cita</button></footer></form></div>

        <div v-if="isBlockModal" class="fixed inset-0 z-50 grid place-items-center bg-[#0F172A]/35 p-4" @click.self="isBlockModal = false"><form class="w-full max-w-lg rounded-lg bg-white shadow-2xl" @submit.prevent="submitBlock"><header class="flex items-center justify-between border-b border-[#D8E0DE] px-5 py-4"><h2 class="font-bold text-[#131B2E]">Bloquear horario</h2><button type="button" @click="isBlockModal = false"><X class="h-5 w-5" /></button></header><div class="grid gap-4 p-5"><input v-model="blockForm.title" required class="h-10 border border-[#BDC9C6] px-3 text-sm" placeholder="Motivo del bloqueo" /><input v-model="blockForm.start_time" type="datetime-local" required class="h-10 border border-[#BDC9C6] px-3 text-sm" /><input v-model="blockForm.end_time" type="datetime-local" required class="h-10 border border-[#BDC9C6] px-3 text-sm" /></div><footer class="flex justify-end gap-2 border-t border-[#D8E0DE] p-4"><button type="button" class="h-10 border border-[#BDC9C6] px-4" @click="isBlockModal = false">Cancelar</button><button class="h-10 bg-[#005C55] px-4 font-semibold text-white">Crear bloqueo</button></footer></form></div>
    </ClinicLayout>
</template>
